...
Run elevated with `./LunarSpiderSim-Complete.ps1 -LabConfirmed`. Artifacts remain in `%PUBLIC%\LunarSpiderSim`; cleanup is separate. No malware, injection, credential access, UAC/persistence change, exploit, scan, remote action, exfiltration, or encryption occurs. Consistent with the report, ransomware is not simulated.
