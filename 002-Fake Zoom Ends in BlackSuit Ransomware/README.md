...
Run elevated with the standard environment gate and `./BlackSuit_Simulation.ps1 -LabConfirmed`. Artifacts remain in `%PUBLIC%\BlackSuitZoomSim`; cleanup is separate. No download, live malware, Defender change, injection, credential/LSASS access, remote service/RDP, external traffic, collection/exfiltration, shadow deletion, or encryption occurs.
