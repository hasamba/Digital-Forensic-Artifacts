...
Run elevated with the standard environment gate and `./GentlemanSim-Complete.ps1 -LabConfirmed`. Artifacts remain in `%PUBLIC%\GentlemanSim`; cleanup is separate. No software is downloaded, IOC traffic leaves the host, credentials are accessed, accounts/tasks/services/GPOs are created, remote systems are touched, security controls or logs/shadows are changed, or files are encrypted.
