...
$expected = Join-Path $env:PUBLIC 'GraceWipeSim'
...
if (-not (Test-Path -LiteralPath $paths.Owner) -or (Get-Content -LiteralPath $paths.Owner -Raw).Trim() -ne $script:GraceWipeId) { throw 'Refusing cleanup of unowned root' }
