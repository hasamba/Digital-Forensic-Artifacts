...
$expected = Join-Path $env:PUBLIC 'SummitDetectSim'
if ($p.Root -ne $expected) { throw 'Cleanup root mismatch' }
if (-not (Test-Path -LiteralPath $p.Root)) { Write-Host 'Nothing to clean.'; return }
if (-not (Test-Path -LiteralPath $p.Owner) -or (Get-Content -LiteralPath $p.Owner -Raw).Trim() -ne $script:SummitDetectId) { throw 'Refusing cleanup of unowned root' }
