...
Run elevated with `./TrigonaSim-Complete.ps1 -LabConfirmed`. Artifacts remain in `%PUBLIC%\TrigonaSim`; cleanup is separate. No authentication, scan, user/account/firewall/Defender/registry change, remote RDP/SMB action, exfiltration, shadow deletion, or encryption occurs; the published Support password is deliberately not copied into generated artifacts.
